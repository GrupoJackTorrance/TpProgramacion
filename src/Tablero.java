...
// 						 ------------	M E T O D O S    -------------
	 ...
	 * posiciones en las que no se creó ningun objeto toman el valor null. Se
	 * hardcodeó para poder hacer las pruebas con un tablero que conozcamos, con
	 * bifurcaciones.
	 */
	public Tablero() {
	...
	/*
	 * COMENTO ESTO PARA VER SI PUEDO DESARROLAR ESOS METODOS DE OTRA FORMA public
	 * boolean avanzarJugador(int posiciones, Jugador miJugador) { int posX=0, posY,
	 * i=0, j=0;; Scanner in;
	 * 
	 * while(posiciones>0) {
	 * 
	 * // Las variables i y j corresponden a las coordenadas de la posición del
	 * jugador //(x e y) // Comienzo evaluando si hay alguna bifurcación a tomar.
	 * //Para esto, hay que chequear que no me salga de la matriz //xq daría null
	 * pointer exception, y verificar alguno de los pares de //casilleros que rodean
	 * al jugador, estén libres (instanciados) //
	 * 
	 * if((j<mapa.length && i<mapa.length && puedeAvanzar(i, j+1) &&
	 * puedeAvanzar(i+1, j)) || (i<mapa.length && j>1 && puedeAvanzar(i+1, j) &&
	 * puedeAvanzar(i, j-1)) || (i>1 && j>1 && puedeAvanzar(i-1, j) &&
	 * puedeAvanzar(i, j-1)) || (i>1 && j<mapa.length && puedeAvanzar(i-1, j) &&
	 * puedeAvanzar(i, j+1))
	 * 
	 * ) { //le pido que ingrese las coordenadas a las que quiere ir
	 * 
	 * System.out.println("Ingrese la posición en X"); in = new Scanner(System.in);
	 * posX = in.nextInt(); System.out.println("Ingresaste x "+posX);
	 * System.out.println("Ingrese la posición en Y"); posY = in.nextInt();
	 * System.out.println("Ingresaste x "+posY);
	 * 
	 * //posicionar al jugadoren el par (x,y) ingresado } else { if(j<4 &&
	 * puedeAvanzar(i, j+1) && verificarPosAnterior(i, j+1)){ //Verifico que no
	 * salga de la matriz, que el casillero este libre, y que no haya pasado por ahí
	 * en el avance anterior //me muevo a la derecha: incremento en 1 la posicion en
	 * j del jugador } else if(i<4 && puedeAvanzar(i+1, j) &&
	 * verificarPosAnterior(i+1, j)){ //me muevo para abajo: incremento en 1 la
	 * posicion en i del jugador } else if(j>1 && puedeAvanzar(i, j-1) &&
	 * verificarPosAnterior(i, j-1)){ //me muevo a la izquierda: decremento en 1 la
	 * posicion en j del jugador } else if(i>1 && puedeAvanzar(i-1, j) &&
	 * verificarPosAnterior(i-1, j)){ //me muevo para arriba: decremento en 1 la
	 * posicion en i del jugador }
	 * 
	 * posiciones--; } }
	 * 
	 * return true; }
	 * 
	 * public boolean puedeAvanzar(int x, int y) { if(this.mapa[x][y] == null)
	 * return false; return true; }
	 * 
	 * // Este método sirve para que el algoritmo no te mande siempre para el mismo
	 * lado. Por defecto, está programado para mandar //al jugador a la derecha y
	 * abajo primero, pero si despues de una bifurcacion queres irte para la
	 * izquierda o para arriba, voy //a necesitar de este método que va a evitar que
	 * vuelva a la posicion anterior. //public boolean verificarPosAnterior(int x,
	 * int y) { if(x == posicionAnteriorEnX && y == posicionAnteriorEnY) return
	 * false; return true; }
	 */
